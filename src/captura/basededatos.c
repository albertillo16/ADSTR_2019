 ...
 *						-----------------------------------------------
 El programa guarda los datos de temperatura, humedad, fecha y inclinación de la placa solar  en una base de datos.
 ...
 */
  ...
  *			\brief Función que se encarga de insertar los datos de humedad, temperatura, la fecha-hora actual y los grados de incliniación de la placa solar.
  ...
  *			@return No devuelve ningún valor.
	\verbatim
			...
			memset(sql, '\0', sizeof(sql[0]));
			sprintf(sql, "INSERT INTO Datos_lecturas (ID, Grados, Unidad, Fecha_hora_lectura)"\
						"VALUES (3, %3.2f,'%s','%s');", *acc_Yangle, grados_unidad, tiempo_actual); 			
			rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);
  ...
  *		- Valores máximo y mínimos humedad, temperatura y grados de incliniación.
  ...
  *	\return No devuelve ningún valor.
	...
	\verbatim
			...
			memset(sql, '\0', sizeof(sql[0]));
			sprintf(sql, "SELECT MAX(Grados),Fecha_hora_lectura FROM Datos_lecturas "\
									"WHERE ID=3 AND Fecha_hora_lectura > '%s'", tiempo_anterior);
			rc = sqlite3_exec(db, sql, callback_alarma, (void *)data, &zErrMsg);
			printf("La información que contiene data es %s\n", data);
			valor_alarma = strtok(data, separador);
			date_alarma = strtok(NULL, separador);
			values_grados= atof(valor_alarma);
			if (values_grados > 59.3818){
				memset(sql, '\0', sizeof(sql[0]));
				sprintf(sql, "INSERT INTO Datos_alarm (Motivo, Fecha_hora_alarm)"\
							"VALUES('Se recomienda cambiar el angulo de inclinación a 59 ', '%s');", date_alarma);
			}
			memset(sql, '\0', sizeof(sql[0]));
			sqlite3_close(db);
	}
...
int insertar_basededatos(char *rutabase_datos, float *humidity, float *temperature, char* tiempo_actual, float *acc_Yangle) {
...
int alarm_basededatos(char *rutabase_datos, char *tiempo_anterior) {
	...
	memset(sql, '\0', sizeof(sql[0]));
	sprintf(sql, "SELECT MIN(Grados),Fecha_hora_lectura FROM Datos_lecturas "\
							"WHERE ID=3 AND Fecha_hora_lectura > '%s'", tiempo_anterior);
	rc = sqlite3_exec(db, sql, callback_alarma, (void *)data, &zErrMsg);
	printf("La información que contiene data es %s\n", data);
	valor_alarma = strtok(data, separador);
	...
	if (values_grados <  0){
		memset(sql, '\0', sizeof(sql[0]));
		sprintf(sql, "INSERT INTO Datos_alarm (Motivo, Fecha_hora_alarm)"\
					"VALUES('Lectura erronea', '%s');", date_alarma);
		rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);
	}
	memset(sql, '\0', sizeof(sql[0]));
	sqlite3_close(db);
