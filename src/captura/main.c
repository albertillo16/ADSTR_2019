 ...
 * Para cada valor de temperatura,humedad y inclinación de la placa solar se guardarán en una base de datos, como tambien si ha ocurrido una alarma.
 * El programa consta de 3 librerias dinámicas:
 ...
 *
	...
	\fn Acc_sensor(&acc_Yangle);
	\fn Humidity_temperature_sensor(&humidity, &temperature);
	\fn insertar_basededatos(rutabase_datos, &temperature, &humidity, tiempo_actual, &acc_Yangle);	
 ...
 \verbatim
	...
	contadormin++;
		if (contadorseg == segundos){
			...
			Acc_sensor(&acc_Yangle);
			Humidity_temperature_sensor(&humidity, &temperature);
			insertar_basededatos(rutabase_datos, &temperature, &humidity, tiempo_actual, &acc_Yangle);
			contadorseg =0;
		}
