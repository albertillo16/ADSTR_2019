 ...
 *				2. Ejecuta la lectura de la humedad, la temperatura y los grados de inclinación cada xx segundos.
 */
 /**
 * @file				hum_temp.c
 *
 ...
 *						-----------------------------------------------
 \verbatim
	...
	\def #define WHO_AM_I 0x0F
		...
		@note Los registros en el rango de direcciones 0x30 a 0x3F contienen coeficientes de calibración.
		...
		@note Variables definidas para pasar radianes a grados.
		
				#define RAD_TO_DEG 57.29578
				#define M_PI 3.14159265358979323846
		@note Registros para activar el sensor del accelerometro
				#define LSM9DS1_ACC_ADDRESS 0x6A
				#define LSM9DS1_WHO_AM_I_XG 0x0F
				#define LSM9DS1_WHO_AM_I_AG_RSP 0x68
		@note Registros para activar el accelerometro en los ejes x, y, z.
				#define LSM9DS1_CTRL_REG5_XL 0x1F
				#define LSM9DS1_CTRL_REG6_XL 0x20

		@note Registros para leer la información del accelerometro de los 3 ejes.
		@note Cada eje tiene LSB y MSB.
 ...
 */
  ...
  *		@param Los parámetros introducidos son propios del programa main.c
  ...
  *		@param Los párametros introducidos son propios de esta función normalizada.
  *		\return No devuelve ningún valor
  *
  ...
  *
  * \fn int Acc_sensor(float *acc_Yangle)
  * 	\brief Función normalizada para visualizar los grados de inclinación de la palca solar.
  * 	@param Los parámetro introducidos son propios del programa main.c
  * 	\return Devuelve los grados de inclinación a través de punteros.
  * 
  * \fn void readBlock(uint8_t command, uint8_t size, uint8_t *data, int fx)
  * 	\brief Función normalizada para leer los datos del accelerometro de los 3 ejes.
  * 	@param Los parámetros introducidos son del propio programa.
  * 	\return No devuelve ningún valor.
  */
 ...
 */
  ...
  * \fn int Acc_sensor(float *acc_Yangle){
		\verbatim
				int fdfd = 0;
				int accRaw[3];
				int LSM9DS1_WHO_XG_RESPONSE = 0;
				int result = 0;
				uint8_t block[6];
				if ((fdfd = open(DEV_PATH, O_RDWR)) < 0) {
					perror("Error al abrir el puerto I2C");
					exit(1);
					
				}
				if (ioctl(fdfd, I2C_SLAVE, LSM9DS1_ACC_ADDRESS) < 0){
					printf("Failed to select I2C device\n");
				}
				LSM9DS1_WHO_XG_RESPONSE = i2c_smbus_read_byte_data(fdfd, LSM9DS1_WHO_AM_I_XG);
				
				if (LSM9DS1_WHO_XG_RESPONSE == LSM9DS1_WHO_AM_I_AG_RSP) {
					printf("LSM9DS1 detected\n");
				}else{
					printf("LSM9DS1 no detected\n");
				}
				result = i2c_smbus_write_byte_data(fdfd, LSM9DS1_CTRL_REG5_XL, 0x38);
				if (result < 0){
					printf("Failed to write byte to I2C acc\n");
				}
				result = i2c_smbus_write_byte_data(fdfd, LSM9DS1_CTRL_REG6_XL, 0x28);
				if (result < 0){
					printf("Failed to write byte to I2C acc\n");
				}
				
				readBlock(0x80 | LSM9DS1_OUT_X_L_XL, sizeof(block), block, fdfd);
				
				*accRaw = (int16_t) (block[0] | block[1] << 8);
				*(accRaw+1) = (int16_t) (block[2] | block[3] << 8);
				*(accRaw+2) = (int16_t) (block[4] | block[5] << 8);
				*acc_Yangle = (float) (atan2 (accRaw[2], accRaw[0])+M_PI) * RAD_TO_DEG;
					if(*acc_Yangle >90){
						*acc_Yangle -= (float)270;
					}
					*acc_Yangle += (float)90;
				close(fdfd);
				return 0;
 ...
 * \fn void readBlock(uint8_t command, uint8_t size, uint8_t *data, int fx){
		\verbatim
			int result = 0;
			result = i2c_smbus_read_i2c_block_data(fx, command, size, data);
			if (result != size) {
